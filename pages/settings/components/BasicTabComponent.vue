  ...
  <v-card flat>
    ...
    <v-card class="mt-2 mx-2">
      <v-list flat>
        ...
        <v-list-item>
          <v-list-item-icon><v-icon>mdi-timer-10</v-icon></v-list-item-icon>
          <v-list-item-content>
    ...
    <v-card class="mt-2 mx-2">
      <v-list flat>
        ...
        <v-list-item>
          <v-list-item-icon><v-icon>mdi-quality-high</v-icon></v-list-item-icon>  
          <v-list-item-content>
...
export default {
  ...
  watch: {
    ...
    currentImageFormat(newValue) {
      this.store.setValue(Settings.SCREENSHOT_FORMAT, newValue);
      if (newValue === 'image/png') {
          this.currentImageQuality = 100;
      }
    },
    currentImageQuality(newValue) {
      this.store.setValue(Settings.SCREENSHOT_QUALITY, newValue);
    },
  },
  methods: {
    async initStore() {
      ...
      this.currentImageFormat = this.store.getValue(Settings.SCREENSHOT_FORMAT, 'image/png') || 'image/png';
      this.currentImageQuality = this.store.getValue(Settings.SCREENSHOT_QUALITY, 100) || 100;
    }
  },
  mounted() {
    this.initStore();
